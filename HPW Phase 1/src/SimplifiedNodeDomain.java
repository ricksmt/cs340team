 ...
 * 
 * downPointers are also known as surrogateNeighbors upPointers are also known
 * as edgeNodes
 * 
 * <pre>
 *  <b>Domain:</b>
...
public class SimplifiedNodeDomain {
	 ...
	 * 
	 * @param webId
	 *            the simplifiedNodeDomain's new webId.
	 * @param height
	 *            the simplifiedNodeDomain's new height.
	 * @param neighbors
	 *            the simplifiedNodeDomain's new neighbors.
	 * @param upPointers
	 *            the simplifiedNodeDomain's new upPointers.
	 * @param downPointers
	 *            the simplifiedNodeDomain's new downPointers.
	 * @param fold
	 *            the simplifiedNodeDomain's new fold.
	 * @param surrogateFold
	 *            the simplifiedNodeDomain's new surrogateFold.
	 * @param inverseSurrogateFold
	 *            the simplifiedNodeDomain's new inverseSurrogateFold.
	 * 
	 ...
	 * 
	 *       In other words:<br>
	 *       1. The simplifiedNodeDomains webId equals the webId passed in. 1.
	 *       The simplifiedNodeDomains height equals the height passed in. 2.
	 *       The simplifiedNodeDomains neighbors equals the neighbors passed in.
	 *       3. The simplifiedNodeDomains upPointers equals the upPointers
	 *       passed in. 4. The simplifiedNodeDomains downPointers equals the
	 *       downPointers passed in. 5. The simplifiedNodeDomains fold equals
	 *       the fold passed in. 6. The simplifiedNodeDomains surrogateFold
	 *       equals the surrogateFold passed in. 7. The simplifiedNodeDomains
	 *       inverseSurrogateFold equals the inverseSurrogateFold passed in.
	 */
	public SimplifiedNodeDomain(int webId, int height,
			HashSet<Integer> neighbors, HashSet<Integer> upPointers,
			HashSet<Integer> downPointers, int fold, int surrogateFold,
	 ...
	 * The equals operator.
	 * 
	 * @param otherSimplifiedNodeDomain
	 *            the other object we are comparing against
	 * @pre <i>None</i>
	 * @post result = otherSimplifiedNodeDomain &ne; null AND<br>
	 *       otherSimplifiedNodeDomain is a simplifedNodeNodeDomain AND<br>
	 *       the webIds, neighbors, downPointers, fold, surrogateFold, and
	 *       inverseSurrogateFold of this and the otherSimplifiedNodeDomain are
	 *       equal.
	 */
	public boolean equals(Object otherSimplifiedNodeDomain) {
		...
		if (result) {
			...
			result = webId == SimplifiedNodeDomain.webId
					...
					&& setEquals(downPointers,
							SimplifiedNodeDomain.downPointers)
	...
	/**
	 * Returns the <i>upPointers</i>. Also known as the <i>upPointers</i>
	 * getter.
	 * 
	 * @pre <i>None</i>
	...
	/**
	 * Determines whether there is an id in neighbors, upPointers, downPointers,
	 * fold, surrogateFold, or inverseSurrogateFold that is closer to the
	 * targetId than the nextNodeId. Closer is defined as the number of
	 * locations in their webId's binary representation in which their
	 * respective bits differ.
	 * 
	 * @param nextNodeId
	 *            the id we think is as close to the target as any other node
	 *            known to this node.
	 * @param targetId
	 *            the id we are computing the distance to.
	 * 
	 * @pre <i>None</i>
	 * @post result = NOT &exist; nodeId ((nodeId &isin; neighbors OR nodeId
	 *       &isin; upPointers OR nodeID &isin; downPointers OR<br>
	 *       nodeID = fold OR nodeId = surrogateFold OR nodeId =
	 *       inverseSurrogateFold) AND nodeId &ge; 0 AND
	 *       nodeId.distanceTo(targetId) < nextNodeId.distanceTo(targetId))
	 */
	public boolean containsCloserNode(int nextNodeId, int targetId) {
		...
		result = result
				|| (distanceTo(inverseSurrogateFold, targetId) < distance);
	 ...
	 * 
	 * @param thisId
	 *            the first id used in the computation.
	 * @param otherId
	 *            the second id used in the computation
	 * 
	 * @pre <i>None</i>
	 * @post result = NumberOf 0 &le; i &le; 31 (thisId.binaryRepresentation[i]
	 *       &ne; otherId.binaryRepresentation[i])
	 */
	 ...
	 * 
	 * @param set1
	 *            the first set in the comparison.
	 * @param set2
	 *            the second set in the comparison.
	 * 
	...
	/**
	 * Appends a hashSet of Integer to the end of the contents of the
	 * stringBuffer. The hashSet is represented as a list of space separated
	 * integers such that each one is less than or equal to the next integer.
	 * 
	 * @param stringBuffer
	 *            the buffer we are going to append the list of integer to.
	 * @param set
	 *            the set we are going to create a sorted list from.
	 * 
	 * @pre stringBuffer &ne; null AND set &ne; null;
	 * @post result = stringBuffer + the string representation of the integers
	 *       in the set such that all integers are separated by a " ". The
	 *       integers are also sorted from lowest to highest.
	 */
	protected void appendSortedListOf(StringBuffer stringBuffer,
			HashSet<Integer> set) {
