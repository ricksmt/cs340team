 ...
 */
public class WebId {
	// I don't know why the previous line shows as yellow in the coverage
	// report. It is not even
	...
	/**
	 * The standard webId constructor that creates a new web id from the given
	 * id.
	 * 
	 * @param id
	...
	public WebId(int id) {
		...
		// The invariant of a constructor should never be false. Thus the
	...
	public WebId(int id, int height) {
		assert id >= 0 && height >= locationOfMostSignificantOneBit(id) + 1
				&& height <= 31;
		this.id = id;
		this.height = height;
		mask = ~(-1 << height);
	 ...
	 *       this &ne; NULL_WEB_ID AND height > 0 &rArr; |result| = height AND
	 *       result = Integer.valueOf(result, 2)
	 */
	 ...
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp; object instanceof WebId AND <br>
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp; id = object.id AND<br>
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp; height = object.height
	 */
	 ...
	 * 
	 * @param webId
	 *            the possible neighbor we are comparing against.
	 * 
	 ...
	 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	 * y.toString().charAt(y.toString()-1) = '1' AND -- the high order bit of y
	 * is 1. Because the height of x is shorter its corresponding bit is 0.<br>
	 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	 * x.distanceTo(y) = 2 -- That is, the bits at location webId.height of x
	 * and y differ plus one and only one other bit.
	 * 
	 * @param webId
	 *            The other webId we are going to compare against.
	 * 
	 * @pre webId &ne; null AND <br>
	 *      webId &ne; NULL_WEB_ID AND <br>
	 *      this &ne; NULL_WEB_ID
	 ...
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp; webId.id &amp; webId.highOrderBit &ne; 0 AND <br>
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp; (~(-1 << height) &amp; webId.Id) < id AND <br>
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp; distanceTo(webId) = 2<br>
	 * <br>
	 */
	public boolean isSurrogateNeighborOf(WebId webId) {
		...
		boolean result = height + 1 == webId.height
				...
				&& (mask & webId.id) < id && distanceTo(webId) == 2;
	 ...
	 * 
	 * @param webId
	 *            The webId we are going to test against.
	 * 
	 * @pre webId &ne; null AND <br>
	 *      webId &ne; NULL_WEB_ID AND <br>
	 *      this &ne; NULL_WEB_ID
	 * @post result = height = webId.height &rArr; result = (~(-1 << height)
	 *       &amp; id) = (~(-1 << webId.height) &amp; ~webId.id) AND<br>
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp; height + 1 = webId.height + 1 &rArr; result = (~(-1 <<
	 *       height) &amp; id) = (~(-1 << height) &amp; ~webId) AND<br>
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp; height = webId.height + 1 &rArr; result = (~(-1 <<
	 *       webId.height) &amp; id) = (~(-1 << webId.height) &amp;
	 *       ~webId.webId) AND<br>
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp; height &ne; webId.height && height + 1 &ne; webId.height &&
	 *       height &ne webId.id + 1 &rArr; result = false )
	 */
	public boolean isFoldOf(WebId webId) {
		...
		if (id != -1 && webId.id != -1) {
			...
			} else if (height + 1 == webId.height
					&& (webId.id & ~webId.highOrderBitMask) < id) {
				result = (mask & id) == (mask & ~webId.id);
			} else if (height == webId.height + 1
					&& (id & ~highOrderBitMask) < webId.id) {
	 ...
	 * @post result = height + 1 = webId.height AND<br>
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp; webId.id < id AND<br>
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp; (~(-1 << height) &amp; webId.id = ~(~(-1 << height) &amp; id)
	 */
	public boolean isSurrogateFoldOf(WebId webId) {
		assert webId != null && webId != NULL_WEB_ID && id >= 0;

		boolean result = (height + 1 == webId.height) && (webId.id < id)
	 ...
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp; id < webId.id AND<br>
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp; (~(-1 << webId.height) &amp; webId.id = ~(~(-1 <<
	 *       webId.height) &amp; id)
	 */
	public boolean isInverseSurrogateFoldOf(WebId webId) {
		assert webId != null && webId != NULL_WEB_ID && id >= 0;

		boolean result = (height == webId.height + 1) && (id < webId.id)
	 ...
	 * 
	 * @param webId
	 *            The webId we are going to test to see if it satisfies the
	 *            invariant.
	 * 
	 * @pre webId &ne; null
	 * @post result = webId.id &ge; -1 AND<br>
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp; webId.height &ge; -1 AND<br>
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp; webId.height &le; MAX_HEIGHT AND<br>
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp; (webId.id = -1 OR webId.height = -1) &rArr; webId =
	 *       NULL_WEB_ID AND<br>
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp; webId.id &ge; 0 &rArr; webId.height &ge;
	 *       &lceil;log<sub>2</sub>(webId.id + 1)&rceil;
	 */
	private static boolean invariant(WebId webId) {
		// This is only used to verify that the constructors, especially the
		// default constructor, are
		// implemented correctly. Thus the webId will never be null. Thus, the
		// coverage report will
		// show the following line a yellow.
		assert webId != null;

		// If operations are all implemented correctly, the result should always
		// be true. This can only
		// be the case if all the conjunctions are true. Thus, none of the
		// conditions are false.
		// This will show in the coverage report as the last conjunction being
		// yellow.
	...
	/**
	 * Returns the location of the highest one bit in the id. Normally this
	 * method is private but we made it public so we can demonstrate loop
	 * testing.
	 * 
	 * @param id
	 *            We will find the location of the highest one bit of this id.
	 * @pre id &ge; 0
	 * @post id = 0 &rArr; result = -1 AND<br>
	 *       id > 0 &rArr; Integer.toBinaryString(id).charAt(result) = '1' AND <br>
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp;&nbsp;&nbsp; &forall; result &lt; i < 32
	 *       (Integer.toBinaryString(id).charAt(i) = '0')
	 */
	public static int locationOfMostSignificantOneBit(int id) { // DataFlow
																// testing line
																// 0.
		// At the three locations where this method is invoked, the id is >= 0.
		// Thus, this pre-condition
		// is always true. Thus, the coverage report would normally show this as
		// never being false.
		// However, we have made this public for loop testing purposes, so we
		// can force the assertion to be both
		// true and false.
	...
	/**
	 * Returns the number of locations in which the bits of this id differ from
	 * the bits in the other webId's id.
	 * 
	 * @param webId
	 *            the other webId whose id we are going to compare this id
	 *            against.
	 * 
	 * @pre webId &ne; null AND <br>
	 *      id &ge; 0 AND <br>
	 *      webId.id &ge; 0
	 * @post result = NumberOf 0 &le; i &lt; min(height, webId.height)
	 *       (id.toString().charAt(i) &ne; webId.toString().charAt(i)) + <br>
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp; NumberOf min(height, webId.height) &le; i &lt; height
	 *       (toString().charAt(i) = '1') + <br>
	 *       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
	 *       nbsp; NumberOf min(height, webId.height) &le; i &lt; webId.height
	 *       (webId.toString().charAt(i) = '1')
	 */
	private int distanceTo(WebId webId) {
		// The pre-conditions of the two methods where this method are invoked
		// guarantee
		// that this pre-condition is always true. Thus, the coverage report
		// will always show this as
		// yellow.
	...
	// This checks the invariant of the default constructor which is only used
	// to create the NULL_WEB_ID.
	// With this implementation it is impossible to make the assertions false.
	// Thus, the coverage report
