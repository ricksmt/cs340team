...
public class Exp {
    ...
    private static boolean testAddingAndRemovingNeighbors(Node node,
        ...
        if (simpleDomain.getNeighbors().size() != 1) {
            System.err.println("When invoking addNeighbor(Node) on a node with no neighbors the number of neighbors should be 1 but is "
        ...
        if (simpleDomain.getNeighbors().size() != 3) {
            System.err.println("After adding node0, node1, and node2 as neighbors to node, the number of neighbors should be 3 but is "
                            + simpleDomain.getNeighbors().size() + ".");
        }
        if (!simpleDomain.getNeighbors().contains(0)) {
    ...
    private static boolean testAddingAndRemovingUpPointers(Node node, boolean error) {
        Node node0 = new Node(0);
        Node node1 = new Node(1);
        Node node2 = new Node(2);
        ...
        if (simpleDomain.getUpPointers().size() != 1) {
            System.err.println("When invoking addUpPointer(Node) on a node with no upPointers the number of upPointers should be 1 but is "
        ...
        if (simpleDomain.getUpPointers().size() != 3) {
            System.err.println("After adding node0, node1, and node2 as upPointers to node, the number of upPointers should be 3 but is "
                            + simpleDomain.getUpPointers().size() + ".");
        }
        if (!simpleDomain.getUpPointers().contains(0)) {
    ...
    private static boolean testAddingAndRemovingDownPointers(Node node, boolean error) {
        Node node0 = new Node(0);
        Node node1 = new Node(1);
        Node node2 = new Node(2);
    ...
    private static void testTheCreationOfAHyPeerWebFromAnExistingDababase() {
        ...
        hypeerweb.reload();
        if (hypeerweb.size() != 3) {
            System.err.println("After reloading the original database, the HyPeerWeb should have 3 nodes in it but it has "
                            + hypeerweb.size() + ".");
            error = true;
            errorCount++;
        }
        ...
        if (node == null) {
            System.err.println("After reloading the original database, node 0 was not found."
                            + hypeerweb.size() + ".");
            error = true;
            errorCount++;
        }
            ...
            if (simpleDomain.getNeighbors().size() != 2) {
                System.err.println("After reloading the original database, there should be only 2 neighbors of node 0 but there are "
                                + simpleDomain.getNeighbors().size() + ".");
                error = true;
                errorCount++;
            }
            ...
            if (simpleDomain.getUpPointers().size() != 0) {
                System.err.println("After reloading the original database, node 0 should have no UpPointers but has "
                                + simpleDomain.getUpPointers().size() + ".");
                error = true;
                errorCount++;
            }
            if (simpleDomain.getDownPointers().size() != 0) {
                System.err.println("After reloading the original database, node 0 should have no DownPointers but has "
                                + simpleDomain.getDownPointers().size() + ".");
                error = true;
                errorCount++;
            }
        ...
        if (node == null) {
            System.err.println("After reloading the original database, node 1 was not found."
                            + hypeerweb.size() + ".");
            error = true;
            errorCount++;
        }
            ...
            SimplifiedNodeDomain simpleDomain = node.constructSimplifiedNodeDomain();
            if (simpleDomain.getNeighbors().size() != 1) {
                System.err.println("After reloading the original database, there should be only 1 neighbor of node 1 but there are "
                                + simpleDomain.getNeighbors().size() + ".");
                error = true;
                errorCount++;
            }
            ...
            if (simpleDomain.getUpPointers().size() != 1) {
                System.err.println("After reloading the original database, node 1 should have 1 UpPointer but has "
                                + simpleDomain.getUpPointers().size() + ".");
                error = true;
                errorCount++;
            }
            ...
            if (simpleDomain.getDownPointers().size() != 0) {
                System.err.println("After reloading the original database, node 1 should have no DownPointers but has "
                                + simpleDomain.getDownPointers().size() + ".");
                error = true;
                errorCount++;
            }
        ...
        if (node == null) {
            System.err.println("After reloading the original database, node 2 was not found."
                            + hypeerweb.size() + ".");
            error = true;
            errorCount++;
        }
            ...
            SimplifiedNodeDomain simpleDomain = node.constructSimplifiedNodeDomain();
            if (simpleDomain.getNeighbors().size() != 1) {
                System.err.println("After reloading the original database, there should be only 1 neighbor of node 2 but there are "
                                + simpleDomain.getNeighbors().size() + ".");
                error = true;
                errorCount++;
            }
            ...
            if (simpleDomain.getUpPointers().size() != 0) {
                System.err.println("After reloading the original database, node 2 should have no UpPointers but has "
                                + simpleDomain.getUpPointers().size() + ".");
                error = true;
                errorCount++;
            }
            if (simpleDomain.getDownPointers().size() != 1) {
                System.err.println("After reloading the original database, node 2 should have 1 DownPointer but has "
                                + simpleDomain.getDownPointers().size() + ".");
                error = true;
                errorCount++;
            }
    ...
    static private void testPhase2() {
        ...
        else System.out.println("    No Insertion Errors: Phase 2 Score = 100%");
    }
    ...
    static private void testPhase3() {
        ...
        for (int size = 1; size <= HYPEERWEB_SIZE; size++) {
            ...
            for (int startNodeId = 0; startNodeId < size - 1; startNodeId++) {
                for(int id = 0; id < size; id++){
                    ...
                    hypeerweb.removeFromHyPeerWeb(id);
        ...
        else System.out.println("    No Deletion Errors: Phase 3 Score = 100%");
    }
    ...
    private static void printErrorMessage(int size, SimplifiedNodeDomain simplifiedNodeDomain, ExpectedResult expectedResult) {
        errorCount++;

        System.out.println("-------------------------------------------------------------");
        System.out.println("Deletion ERROR when HyPeerWebSize = " + size + ".");
        System.out.println("Node Information:");
        System.out.println(simplifiedNodeDomain);
        System.out.println();
        System.out.println("Expected Information");
