...
public class Exp{
    public static HyPeerWeb hypeerweb;
    ...
    private static void testSimpleNodeOperationsOnNodeBeforeItHasBeenAddedToHyPeerWeb() {
        ...
        if (simpleDomain.getFold() != -1) {
            //test case was expecting node fold to be self but we initialie fold to be NULL_NODE with id=-1
            System.err.println("When invoking Node(int) constructor as 'new Node(3)', the fold should be -1.");
            error = true;
            errorCount++;
        }
    ...
    private static boolean testAddingAndRemovingNeighbors(Node node, boolean error)
    {
    ...
    private static void testTheCreationOfAHyPeerWebFromAnExistingDababase() throws SQLException, ClassNotFoundException, IOException
    {
        boolean error = false;
        hypeerweb.clear();
